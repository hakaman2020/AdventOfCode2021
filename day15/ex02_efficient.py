...
#large_graph = graph
max_x = len(large_graph)
max_y = len(large_graph[0])
large_graph[0][0].min_dist = 0
coor_list = set()
next_coor = (0,0)
coor_list.add (next_coor)
done = False
while not done:
	dijkstra(large_graph, max_x, max_y, next_coor,coor_list)
	next_coor = return_node_lowest_min_dist(large_graph,coor_list)
	if next_coor == [-1,-1]:
		done = True
	
#print_graph_risk(graph)
#print_graph_visit(graph)
print(large_graph[max_x-1][max_y -1].min_dist)